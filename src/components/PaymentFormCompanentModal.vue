  ...
  <div id="paymentForm">
    <div id="solid-payment-form-container" class="flex justify-between flex-wrap pb-32px">
      <button
        ...
        :class="{ 'border-2px border-[#5773D6 border-solid]': paymentMethodType == 3 }" v-if="apple_pay"
        @click="applePaySelect">
        <img class="w-full" src="@/assets/images/svg/icon_apple-pay.svg" alt="apple_pay" />
      </button>
      <div class="flex">
        <button class="flex items-center flex-1 mr-5px bg-[#F9F9F9] rounded-9px border-2px border-[#fff]"
          :class="{ 'bg-[#5773D61A] !border-blue': paymentMethodType == 2 }" @click="payPalSelect">
          <img class="w-full" src="@/assets/images/icons/icon_paypal.png" alt="apple_pay" />
        </button>
        <button class="flex items-center flex-1 bg-[#F9F9F9] rounded-9px border-2px border-[#fff]"
          :class="{ 'bg-[#5773D61A] !border-blue': paymentMethodType == 1 }" @click="cardSelect">
    ...
    <div class="py-16px px-0 border-t-1px border-[#F1F3F9] border-b-1px grid grid-cols-[2fr_1fr] gap-y-10px">
      <p class="text-14px leading-normal text-[#11111380]">{{ subscription }}</p>
      <p class="text-14px leading-normal text-[#11111380] text-right tracking-0px">{{ fullPrice }}</p>
      <p class="text-14px leading-normal text-[#11111380]">{{ $t('paymentForm.discount', { discount }) }}</p>
      <p class="font-700 text-14px leading-normal text-[#11111380] text-right tracking-0px"
        :class="!theme ? '!text-red' : '!text-blue'">
        -{{ discountAmount }}
      </p>
    </div>
    <div
      class="py-16px px-0 border-t-1px border-[#F1F3F9] border-b-1px grid grid-cols-[0.7fr_2fr] gap-y-10px border-none">
      <p class="font-700 text-18px leading-normal text-[#000]">{{ $t('paymentForm.total') }}</p>
      <p class="font-700 text-18px leading-normal text-[#000] text-right">{{ discountPrice }}</p>
      <p class="text-12px leading-normal text-[#11111380]">{{ $t('paymentForm.perDate', { subscriptionDate }) }}</p>
      <p class="font-700 text-12px leading-normal text-[#11111380] text-right tracking-0px"
        :class="!theme ? '!text-red' : '!text-blue'">
        {{ $t('paymentForm.saved', { discountAmount }, { discount }) }}
      </p>
    </div>
  </div>
  <PayPalComponent class="sticky pt-18px" v-if="paymentMethodType == 2" @error="error" @success="success"
    @clickButton="clickButton" :item="this.item" :discountPrice="discountPrice" :fullPrice="fullPrice"
    :subscriptionDate="subscriptionDate" />
  <div v-if="paymentMethodType == 2"
    class="flex flex-row items-center justify-center pt-12px text-[#111113] opacity-50 text-12px font-600 uppercase">
    {{ $t('vPopup.pay') }}
  </div>
  <CardCompanentModal v-if="paymentMethodType == 1" @error="error" @success="success" @clickButton="clickButton"
    :item="this.item" :auth_price="this.auth_price" :discountPrice="discountPrice" :fullPrice="fullPrice"
    :subscriptionDate="subscriptionDate" :theme="theme" />
  <!-- <button @click="success">PAYMENT IS SUCCESSFUL</button> -->
  <div id="apple-pay-button" class="w-full inline pt-16px" v-if="paymentMethodType == 3"></div>
</template>
